...
import { Loader2, Plus, Trash2, CheckIcon, Globe } from 'lucide-react';
          ...
          <Card className="p-2 bg-card text-card-foreground gap-6 rounded-xl border py-6 shadow-sm border-gray-200 h-full flex flex-col">
            <CardHeader className="pb-4">
              ...
              {scrapingCompetitors && !analyzing && (
                <CardDescription className="mt-2 flex items-center justify-center gap-2 text-emerald-600">
                  ...
                  <div className="flex items-center gap-3 flex-1">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-600 mt-1">
                        Uses each AI's native search (ChatGPT Search, Claude with search, etc.) for real-time, current brand mentions instead of relying on training data
                      </p>
                    </div>
                  </div>
                  <button
                    onClick={() => onUseWebSearchChange(!useWebSearch)}
                    className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 ${useWebSearch ? 'bg-emerald-500' : 'bg-gray-300'
                      }`}
                  >
                    <span
                        ...
                        <div className="flex items-start justify-between gap-4">
                          ...
                          {!analyzing && !isCustom && (
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                // Find the original index in the unfiltered default prompts
                                const serviceType = detectServiceType(company);
                                const currentYear = new Date().getFullYear();
                                ...
                                const originalIndex = allDefaultPrompts.findIndex(p => p === prompt);
                                if (originalIndex !== -1) {
